...
Furthermore, the data is stored in several different formats. We will standardize this so that everything is the same for all individuals and export a new data frame at the end. 
...
The is one clear outlier, I will filter it then re-plot the data.
...
# Rename latlongs
colnames(tmp_ind)[colnames(tmp_ind)=="Latitude"] <- "GPS.Latitude"
colnames(tmp_ind)[colnames(tmp_ind)=="Longitude"] <- "GPS.Longitude"
...
tmp_ind$timestamp <- excel_numeric_to_date(tmp_ind$Date...Time..GMT., include_time = TRUE, tz="UTC")

```

**THERE ARE ISSUES WITH THE TIMESTAMPS - NOT SOLVED**
...
```{r j6}
# Read in the data
tmp_ind <- read.csv(paste0("data/raw_collar/",tmp$name[i], "_",tmp$year[i],".csv"))#, skip=)
# Remove weird first row
tmp_ind <- tmp_ind[2:nrow(tmp_ind),]

#tmp_ind$Date...Time..GMT.[1]
...
tmp_ind <- tmp_ind[is.na(tmp_ind$GPS.Longitude)==F,]
# Sort the dates
tmp_ind$Date...Time..GMT. <- trimws(tmp_ind$Date...Time..GMT.)

tmp_ind$timestamp <- mdy_hms(tmp_ind$Date...Time..GMT.)
```
