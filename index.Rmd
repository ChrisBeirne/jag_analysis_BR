...
We have `r length(unique(tmp$name))` individuals with data, with between `r min(tmp$year)` and `r max(tmp$year)` years.
...
Most individuals are tracked for one year. 

```{r c01_archive, echo=F, message=F, eval=F}
# 
# # Get animals
# # Vultures
